...
def api_competitor_analyze():
    ...
    Runs the full 5-step Competitor Analysis Agent for a startup.

    The agent executes these steps sequentially, each feeding into the next:
      1. Market Scan       — identifies top 5 real competitors
      2. Deep Dive         — analyzes each competitor's strengths, weaknesses, funding
      3. Positioning Map   — maps where the startup sits vs the competition
      4. Gap Intelligence  — surfaces exploitable gaps and blind spots
      5. Battle Plan       — delivers a concrete competitive strategy
    ...
    try:
        ...
        if steps < 5:
            response.setdefault("warning", f"Agent completed {steps}/5 steps. Partial results returned.")
